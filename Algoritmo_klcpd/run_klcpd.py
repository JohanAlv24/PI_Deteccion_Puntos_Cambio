...
def run_klcpd_exp(dataset):
    for wnd_dim in wnd_dim_list:
        ...
        for data_path in glob.glob(os.path.join(data_dir, '*.mat')):
            ...
            cmd = [
                ...
                '--data_path', data_path,
                '--wnd_dim', str(wnd_dim),
                '--lambda_ae', str(lambda_ae),
                '--lambda_real', str(lambda_real),
            ...
            print('Logging to:', trn_log_path)
            with open(trn_log_path, 'w', encoding='utf-8') as log_file:
                result = subprocess.run(cmd, env=env, stdout=log_file, stderr=subprocess.STDOUT, cwd=os.path.dirname(__file__) or '.', shell=False)
            if result.returncode != 0:
                raise RuntimeError('klcpd.py failed for %s with return code %d' % (data_path, result.returncode))
